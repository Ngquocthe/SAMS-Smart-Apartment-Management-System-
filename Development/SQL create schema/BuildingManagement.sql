...
-- 1. Sửa bảng journal_entries
-- Thêm cột mới
ALTER TABLE building.journal_entries ADD entry_type NVARCHAR(50) NULL;
ALTER TABLE building.journal_entries ADD fiscal_period NVARCHAR(20) NULL;

-- Xóa các ràng buộc cũ liên quan đến cột sắp xóa (Bắt buộc phải xóa constraint trước khi xóa cột)
ALTER TABLE building.journal_entries DROP CONSTRAINT IF EXISTS CK_je_totals_nonneg;
ALTER TABLE building.journal_entries DROP CONSTRAINT IF EXISTS FK_je_reversed_by;

-- Xóa các cột không còn dùng (để giống hình)
ALTER TABLE building.journal_entries 
DROP COLUMN total_debit, total_credit, reversed_by, reversed_date, reversal_entry_id;

-- 2. Sửa bảng journal_entry_lines
-- Xóa các cột không còn dùng
ALTER TABLE building.journal_entry_lines 
DROP COLUMN department, cost_center;