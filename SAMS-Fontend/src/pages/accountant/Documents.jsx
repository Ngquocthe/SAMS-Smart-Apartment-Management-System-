...
export default function AccountantDocuments() {
    ...
    const fetchDocuments = useCallback(async () => {
        setLoading(true);
        try {
            const result = await documentsApi.getAccountingDocuments({
                Title: query.keyword || undefined,
                Category: query.category || undefined,
            });
            setDocuments(Array.isArray(result) ? result : []);
        } catch (error) {
            console.error("Load accountant documents error:", error);
            message.error("Không tải được danh sách tài liệu cho kế toán");
        } finally {
            setLoading(false);
        }
    }, [query]);
            ...
            <div style={{ maxWidth: 1400, margin: "0 auto" }}>
                ...
                <div style={{ marginBottom: 24 }}>
                    <Title level={2} style={{ margin: 0, marginBottom: 16 }}>
                        Tài liệu cho kế toán
                    </Title>
                    <p style={{ color: "#8c8c8c", margin: 0 }}>
                        Xem và tải xuống các tài liệu, quy trình nghiệp vụ dành cho kế toán
                    </p>
                </div>
                                ...
                                description={
                                    <span style={{ color: "#8c8c8c" }}>
                                        {query.keyword || query.category
                                            ...
                                            : "Chưa có tài liệu dành cho kế toán"}
                                    </span>
                                }
                            />
                        </div>
