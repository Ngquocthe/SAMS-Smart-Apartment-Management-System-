...
export default function StaffUpdateForm({
  staffCode,
  ...
  const [schemaName, setSchemaName] = useState(propSchemaName || null);
  ...
  // Load roles when schemaName changes
  useEffect(() => {
    if (!schemaName) {
      setRoles([]);
      return;
  ...
  const populateFormData = useCallback(
    (staffData) => {
      // Store initial buildingId
      const buildingId = staffData.buildingId || null;
      ...
      form.setFieldsValue({
        user: {
          ...
          dob: staffData.dob ? dayjs(staffData.dob) : null,
        ...
        staff: {
          ...
          terminationDate: staffData.terminationDate
            ? dayjs(staffData.terminationDate)
            : null,
          ...
          socialInsuranceNo: staffData.socialInsuranceNo || "",
          baseSalary: staffData.baseSalary || undefined,
          roleId: staffData.roleId || null,
          buildingId: staffData.buildingId || null,
        },
        assignment: {
          roles: staffData.accessRoles || [],
        },
      });

      // Set preview images if available
      if (staffData.avatarUrl) {
        setAvatarPreview(staffData.avatarUrl);
      }
      if (staffData.cardPhotoUrl) {
        setCardPhotoPreview(staffData.cardPhotoUrl);
      }
  ...
  useEffect(() => {
    ...
    // If we have schemaName, load directly
    if (schemaName) {
      (async () => {
        try {
          setLoadingData(true);
          const staffData = await staffApi.getDetail(schemaName, staffCode);
          ...
          console.error(e);
          message.error("Không tải được thông tin nhân sự");
          setLoadingData(false);
        }
      })();
      return;
    ...
    // If no schemaName but we have buildings, try to find staff in all buildings
    if (buildings.length > 0) {
      (async () => {
        try {
          ...
          // Try each building until we find the staff
          for (const building of buildings) {
            if (!building.schemaName) continue;
            try {
              const data = await staffApi.getDetail(
                building.schemaName,
                staffCode
              );
              if (data) {
                staffData = data;
                foundSchemaName = building.schemaName;
                break;
              }
            } catch (e) {
  ...
  }, [staffCode, schemaName, buildings, populateFormData]);
  ...
  const handleFinish = async (values) => {
    if (!staffCode || !schemaName) {
      message.error("Thiếu thông tin mã nhân sự hoặc schema");
      return;
    }
    ...
    try {
      ...
      if (typeof onSubmit === "function") {
        await onSubmit(formData, staffCode);
      } else {
        try {
          await staffApi.update(schemaName, staffCode, formData);
          ...
          console.error(e);
          message.error(e.errorMessage || "Cập nhật nhân sự thất bại");
          return;
        }
      }
    } finally {
